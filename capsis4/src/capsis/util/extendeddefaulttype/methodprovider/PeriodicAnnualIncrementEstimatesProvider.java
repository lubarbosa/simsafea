 ...
 */
public interface PeriodicAnnualIncrementEstimatesProvider {
	
	public static enum GrowthComponent implements TextableEnum {
		Mortality("Mortality", "Mortalit\u00E9"),
		Harvesting("Harvesting", "Recolte"),
		Growth("Growth", "Accroissement"), 
	 ...
	 * interventions.
	 */
	public Estimate<Matrix, SymmetricMatrix, ?> getPAIEstimate(ExtCompositeStand stand, GrowthComponent component, VariableForEstimation variable);

	/**
	 * A default method that returns the mean of the estimate or 0 if the estimate is null<
	 * @param stand
	 * @param component
	 * @param variable
	 * @return
	 */
	public default double getPAIMeanEstimate(ExtCompositeStand stand, GrowthComponent component, VariableForEstimation variable) {
		Estimate<Matrix, SymmetricMatrix, ?> est = getPAIEstimate(stand, component, variable);
		if (est != null) {
