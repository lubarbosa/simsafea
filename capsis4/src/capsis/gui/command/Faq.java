...
public class Faq implements Command {
	...
	public int execute() {
		...
		try {
			...
			// fc-7.8.2018 Make Helper independent from Capsis
			URL url = new URL(Starter.getCapsisURL() + "/documentation/FAQ");
			Helper.showPage(url);

			// Helper.showFAQ();

			// fc-7.8.2018 REMOVED, the following catch section is enough
			// } catch (Exception e) {
			// Log.println(Log.ERROR, "Faq.execute ()", "An Exception/Error
			// occured", e);
			// StatusDispatcher.print(Translator.swap("Shared.commandFailed"));
			// MessageDialog.print(this,
			// Translator.swap("Shared.commandFailed"), e);
			// return 1;
