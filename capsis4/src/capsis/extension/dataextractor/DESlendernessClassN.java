...
/**
 * Numbers of trees per height/diameter classes, based on DEDbhClassN.java. H is
 * reduced of 1.30m to have realistic H/D for small trees.
 * 
 * @author B. Courbaud - July 2001
 */
public class DESlendernessClassN extends DETimeG {
	...
	public String getName() {
		return getNamePrefix() + Translator.swap("DESlendernessClassN.name");
	}
	...
	public String getDescription() {
		return Translator.swap("DESlendernessClassN.description");
	}
...
//			if (settings.percentage) {
				...
				try {
					trees = ((TreeCollection) step.getScene()).getTrees();
				} catch (Exception e) {
				}

				double Ntot = ((NProvider) methodProvider).getN(step.getScene(), trees); 
			...
			while (trees.hasNext()) {
				...
				if (t.getDbh() <= 0)
					continue; // PhD 2009-06-12

				// double slenderness = 100*(t.getHeight ()-1.3) / t.getDbh ();
				// //Height is reduced of 1.30 m to have realistic H/D for small
				// trees
				double slenderness = 100 * t.getHeight() / t.getDbh(); // same
																		// calculation
																		// than
																		// ForEole
				// ~ System.out.println ("tree_"+t.getId ()+" d="+t.getDbh
				// ()+" h="+t.getHeight ()+" slenderness="+slenderness);
				if (slenderness < minThreshold) {
					continue;
				} // fc - 30.4.2003
	...
	public List<String> getAxesNames() {
		...
		String xl = isSet("displayClassNames") ? Translator.swap("Shared.class")
				: Translator.swap("DESlendernessClassN.xLabel");
		v.add(xl);

		String yl = Translator.swap("DESlendernessClassN.yLabel");
		if (isSet("perHectare"))
			yl += " (ha)";
		
		// fc-23.8.2021 Added this else for %
		else if (isSet("percentage"))
			yl += " (%)";
