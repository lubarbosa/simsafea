...
public class DETimeBIOMRAC extends DETimeG {
	...
	public DETimeBIOMRAC(GenericExtensionStarter s) {
		...
		try {
			...
			Log.println(Log.ERROR, "DETimeBIOMRAC.c ()", "Exception during construction", e);
		}
	}
	...
	public boolean matchWith(Object referent) {
		try {
			...
			if (!(mp instanceof StumpBiomProvider)) {
				return false;
			}
			if (!(mp instanceof SoilFineRootBiomProvider)) {
				return false;
			}
			if (!(mp instanceof LitterFineRootBiomProvider)) {
				return false;
			}
			if (!(mp instanceof MediumRootBiomProvider)) {
				return false;
			}
			if (!(mp instanceof CoarseRootBiomProvider)) {
				return false;
			}
	...
	public String getName() {
		return getNamePrefix() + Translator.swap("DETimeBIOMRAC.name");
	}
	...
	public String getDescription() {
		return Translator.swap("DETimeBIOMRAC.description");
	}
	...
	public boolean doExtraction() {
		...
		try {
			...
			do {
				...
				double stumpBiom = ((StumpBiomProvider) methodProvider).getStumpBiom(stand, trees);
				double sFineRBBiom = ((SoilFineRootBiomProvider) methodProvider).getSoilFineRootBiom(stand, trees);
				double lFineRBiom = ((LitterFineRootBiomProvider) methodProvider).getLitterFineRootBiom(stand, trees);
				double medRBiom = ((MediumRootBiomProvider) methodProvider).getMediumRootBiom(stand, trees);
				double coarseRBiom = ((CoarseRootBiomProvider) methodProvider).getCoarseRootBiom(stand, trees);
