...
/**
 * A manager to display double properties (textFields), check user input and
 * apply his choices.
 * 
 * @author F. de Coligny - September 2021
 */
public class DoublePropertyManager extends AbstractPropertyManager {
	...
	// JTextFields to display the double properties
	// Key is propertyName, value is the matching textfield
	public Map<String, JTextField> doublePropertiesTextFields;

	/**
	 * Constructor
	 */
	public DoublePropertyManager() {
	...
	public ColumnPanel getPropertyPanel(String propertyName) {
		...
		Double value = settings.doubleProperties.get(propertyName);

		// Create JTextField
		JLabel label = new JWidthLabel(Translator.swap(propertyName) + " :", 190);
		JTextField textField = new JTextField("" + value.doubleValue(), 5);
		
		doublePropertiesTextFields.put(propertyName, textField);
	...
	/**
	 * Layout the double properties in the given panel. Double properties: add
	...
	public Map<String, ColumnPanel> layoutProperties(ColumnPanel propertiesPanel) {
		...
		return ConfigPropertyLayoutStrategy.layoutProperties(this, settings.doubleProperties.keySet(),
				propertiesPanel);
		...
		Iterator keys = new TreeSet(settings.doubleProperties.keySet()).iterator();
		String prevRoot = "";
		JPanel panel = null;
		JPanel inside = null;
		...
		while (keys.hasNext()) {
			...
			Double value = settings.doubleProperties.get(propertyName); // fc-30.8.2012
...
//			doublePropertiesTextFields.put(propertyName, f);
//
//			if (!extractor.isPropertyEnabled(propertyName))
//				f.setEnabled(false);
...
*/
	...
	public boolean checksAreOk() {
		...
		Iterator keys = doublePropertiesTextFields.keySet().iterator();
		Iterator values = doublePropertiesTextFields.values().iterator();
		while (keys.hasNext() && values.hasNext()) {
			...
			try {
				...
				MessageDialog.print(this, Translator.swap(name) + " " + Translator.swap("DataExtractor.mustBeADouble"));
				return false;
			}
		}
	...
	public void applyConfig(ConfigurableExtractor extractorToBeConfigured) {
		...
		while (keys.hasNext() && values.hasNext()) {
			...
			extractorToBeConfigured.getSettings().doubleProperties.remove(name);
			extractorToBeConfigured.getSettings().doubleProperties.put(name, new Double(f.getText()));
		}
