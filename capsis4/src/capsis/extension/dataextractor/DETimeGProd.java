...
/**
 * Production of Basal area over Time(/age) or over Hdom. including dead trees
 * or not. With possibilty to view annual increment
 * 
...
public class DETimeGProd extends PaleoDataExtractor implements DFCurves {
	...
	public DETimeGProd(GenericExtensionStarter s) {
		...
		try {
			...
			setPropertyEnabled("onlyAliveG", availableAliveGProd); // PhD
																	// 2008-12-19

		} catch (Exception e) {
			Log.println(Log.ERROR, "DETimeGProd.c ()", "Exception during construction", e);
		}
	}
	...
	public boolean matchWith(Object referent) {
		try {
			...
			if (!(mp instanceof ProdGProvider)) {
				return false;
			}
	...
	public String getName() {
		return getNamePrefix() + Translator.swap("DETimeGProd.name");
	}
	...
	public String getDescription() {
		return Translator.swap("DETimeGProd.description");
	}
	...
	private void checkMethodProvider() {
		...
		if (methodProvider instanceof ProdAliveGProvider) {
			availableAliveGProd = true;
		}
	...
	public boolean doExtraction() {
		...
		try {
			...
			if (i.hasNext()) { // if a least one step (... !)
				...
				if (isSet("onlyAliveG") && (methodProvider instanceof ProdAliveGProvider)) { // PhD
																								// 2008-12-19
					read = ((ProdAliveGProvider) methodProvider).getProdAliveG(stand, trees) * coefHa;
				} else {
					read = ((ProdGProvider) methodProvider).getProdG(stand, trees) * coefHa; // fc
																								// -
																								// 24.3.2004
				}
				...
				while (i.hasNext()) { // ... beginning at the second date, if
					...
					date = stand.getDate();
					hdom = ((HdomProvider) methodProvider).getHdom(stand, trees);
					// read = ((ProdGProvider) methodProvider).getProdG (stand,
	...
	public List<String> getAxesNames() {
		 ...
		 * ("DETimeGProd.yLabel")+" (ha)"); } else { v.add (Translator.swap
		 * ("DETimeGProd.yLabel")); }
		 */
		String yLab;
