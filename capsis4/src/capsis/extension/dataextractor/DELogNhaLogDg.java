...
public class DELogNhaLogDg extends DETimeG {
	...
	public String getName() {
		return getNamePrefix() + Translator.swap("DELogNhaLogDg.name");
	}
	...
	public String getDescription() {
		return Translator.swap("DELogNhaLogDg.description");
	}
	...
	public boolean doExtraction() {
		...
		try {
			// per Ha computation
			double coefHa = 10000 / getSceneArea(step.getScene()); // fc-2.6.2020
			...
			for (Iterator i = steps.iterator(); i.hasNext();) {
				...
				double Dg = ((DgProvider) methodProvider).getDg(stand, trees); // (Dg
																				// :
																				// cm)

				double Nha = ((NProvider) methodProvider).getN(stand, trees) * coefHa; // (Nha
																						// =
																						// stems
																						// /
																						// ha)
