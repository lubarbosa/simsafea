...
/**
 * Production of Volume over Time(/age) or over Hdom. Including dead trees. With
 * possibility to view annual increment.
 * 
...
public class DETimeVProd2 extends PaleoDataExtractor implements DFCurves {
	...
	public DETimeVProd2(GenericExtensionStarter s) {
		...
		try {
			...
			// setPropertyEnabled ("onlyAliveV", availableAliveVProd); // PhD
			// 2008-12-19

		} catch (Exception e) {
			Log.println(Log.ERROR, "DETimeVProd2.c ()", "Exception during construction", e);
		}
	}
	...
	public boolean matchWith(Object referent) {
		try {
			...
			if (!(mp instanceof ProdVProvider))
				return false;
	...
	public String getName() {
		return getNamePrefix() + Translator.swap("DETimeVProd2.name");
	}
	...
	public String getDescription() {
		return Translator.swap("DETimeVProd2.description");
	}
	...
	private void checkMethodProvider() {
		...
		// if (methodProvider instanceof ProdAliveVProvider)
		// {availableAliveVProd = true;}
	...
	public boolean doExtraction() {
		...
		try {
			...
			// depending on "showIncrement" button (to be changed in
			...
			if (i.hasNext()) { // if a least one step (... !)
				...
				// if (isSet ("onlyAliveV") && (methodProvider instanceof
				// ProdAliveVProvider)) { // PhD 2008-12-19
				// read = ((ProdAliveVProvider) methodProvider).getProdAliveV
				// (stand, trees) * coefHa;
				// } else {
				read = ((ProdVProvider) methodProvider).getProdV(stand, trees, null) * coefHa; // fc
																								// -
																								// 24.3.2004
				// }
				...
				while (i.hasNext()) { // ... beginning at the second date, if
					...
					date = stand.getDate();
					hdom = ((HdomProvider) methodProvider).getHdom(stand, trees);
					// read = ((ProdVProvider) methodProvider).getProdV (stand,
					// trees) * coefHa; // fc - 24.3.2004
					// if (isSet ("onlyAliveV") && (methodProvider instanceof
					// ProdAliveVProvider)) { // PhD 2008-12-19
					// read = ((ProdAliveVProvider)
					// methodProvider).getProdAliveV (stand, trees) * coefHa;
					// } else {
					read = ((ProdVProvider) methodProvider).getProdV(stand, trees, null) * coefHa; // fc
																									// -
																									// 24.3.2004
					...
					if (isSet("xIsHdom")) { // PhD 2008-12-19
						...
						if (hdom > 0) {
							c1.add(new Double(hdom));
						} else { // PhD 2014-11-25 : after clearcutting, hdom
									// might have been set to 0
	...
	public List<String> getAxesNames() {
		 ...
		 * ("DETimeVProd2.yLabel")+" (ha)"); } else { v.add (Translator.swap
		 * ("DETimeVProd2.yLabel")); }
		 */
		String yLab;
