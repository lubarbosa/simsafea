 ...
 *         2008 / June 2012
 */
public class SLEllipsoidalCrownFraction extends SLCrownPart {

	// public static final SLTag TOP_CROWN_FRACTION = new SLTag
	// ("TOP_CROWN_FRACTION");
	// public static final SLTag BOTTOM_CROWN_FRACTION = new SLTag
	// ("BOTTOM_CROWN_FRACTION");
	...
	/**
	 * Constructor for a 8th of an ellipsoid. The signs of a, b and c determine
	 * which 8th is considered.
	 */
	public SLEllipsoidalCrownFraction(double x0, double y0, double z0, double a, double b, double c) {
		this.x0 = x0;
		this.y0 = y0;
		this.z0 = z0;
		this.a = a;
		...
		// // Tag default value
		// if (isUpper())
		// this.tag = TOP_CROWN_FRACTION;
		// else
		// this.tag = BOTTOM_CROWN_FRACTION;
	 ...
	 */
	public SLEllipsoidalCrownFraction getCopy(double xShift, double yShift, double zShift) {

		SLEllipsoidalCrownFraction copy = new SLEllipsoidalCrownFraction(x0 + xShift, y0 + yShift, z0 + zShift, a, b, c);
	...
	// every years)
	public SLEllipsoidalCrownFraction clone() throws CloneNotSupportedException {
		return (SLEllipsoidalCrownFraction) super.clone();
	}
	...
	public double[] intercept(double xShift, double yShift, double zShift, double elevation, double azimuth) {
		...
		if (inBBox(min, max, v0Shifted, v1, epsilon) && v1.z >= 0) //fa-14.05.2019: added epsilon
			list.add(v1);
		if (inBBox(min, max, v0Shifted, v2, epsilon) && v2.z >= 0) //fa-14.05.2019: added epsilon
			list.add(v2);
		if (inBBox(min, max, v0Shifted, vx0, epsilon) && inEllipsoid(v0Shifted, vx0) && vx0.z > 0) //fa-14.05.2019: added epsilon
			list.add(vx0);
//		if (inBBox(min, max, v0Shifted, vy0) && inEllipsoid(v0Shifted, vy0) && vy0.z > 0)
		if (inBBox(min, max, v0Shifted, vy0, epsilon) && inEllipsoid(v0Shifted, vy0) && vy0.z > 0 && !vy0.equals(vx0,epsilon)) //fa-14.05.2019: to avoid duplicate point if beam intersects ellipsoid axis, epsilon to consider rounding errors
			list.add(vy0);
//		if (inBBox(min, max, v0Shifted, vz0) && inEllipsoid(v0Shifted, vz0) && vz0.z > 0)
		if (inBBox(min, max, v0Shifted, vz0, epsilon) && inEllipsoid(v0Shifted, vz0) && vz0.z > 0 && !vz0.equals(vx0,epsilon) && !vz0.equals(vy0,epsilon)) //fa-14.05.2019: to avoid duplicate point if beam intersects ellipsoid axis, epsilon to consider rounding errors
			list.add(vz0);
		if (inBBox(min, max, v0Shifted, vtarget, epsilon) && inEllipsoid(v0Shifted, vtarget)) //fa-14.05.2019: added epsilon
			list.add(vtarget);
		if (list.size() == 0)
			return null; // intersected the ellipse outside the half quarter
		...
		if (list.size() != 2) {
			...
			// Particular cases
			boolean particularCase1 = (zTop2 == 0); // the part center is at the
													// same z as the
													// target point
			...
			boolean particularCase4 = false;  // fa-14.05.2019: intersection with an axis (tangent to the crown fraction through this axis)
			...
			if (list.size() == 1) {
				...
				} else if (v == vtarget && onBBoxPeriphery(min, max, v, epsilon)) { //fa-14.05.2019: added epsilon
					particularCase3 = true;
				} else if (vy0.equals(vx0,epsilon) || vz0.equals(vx0,epsilon) || vz0.equals(vy0,epsilon)) { //fa-14.05.2019
					particularCase4 = true;
				}
			}

//			if (!particularCase1 && !particularCase2 && !particularCase3) {
			if (!particularCase1 && !particularCase2 && !particularCase3 && !particularCase4) { //fa-14.05.2019
				
			//fa-trace for checking				
//			System.out.println(a + "\t" + b + "\t" + c + "\t" + xShift + "\t" + yShift + "\t" + zShift + "\t" + x0 + "\t" + y0 + "\t" + z0 + "\t" + x2 + "\t" + y2 + "\t" + z2 + "\t" + elevation + "\t" + azimuth + "\t" + root1 + "\t" + root2 + "\t" + v1.x + "\t" + v1.y + "\t" + v1.z + "\t" + v2.x + "\t" + v2.y + "\t" + v2.z + "\t" + vx0.x + "\t" + vx0.y + "\t" + vx0.z + "\t" + vy0.x + "\t" + vy0.y + "\t" + vy0.z + "\t" + vz0.x + "\t" + vz0.y + "\t" + vz0.z + "\t" + list.size() + "\t" + inEllipsoid(v0Shifted,vx0) + "\t" + inEllipsoid(v0Shifted,vy0) + "\t" + inEllipsoid(v0Shifted,vz0) + "\t" + inEllipsoid(v0Shifted, vtarget));

//				if (list.size() == 1)
//					System.out.println(a + "\t" + b + "\t" + c + "\t" + xShift + "\t" + yShift + "\t" + zShift + "\t" + x0 + "\t" + y0 + "\t" + z0 + "\t" + x2 + "\t" + y2 + "\t" + z2 + "\t" + elevation + "\t" + azimuth + "\t" + root1 + "\t" + root2 + "\t" + v1.x + "\t" + v1.y + "\t" + v1.z + "\t" + v2.x + "\t" + v2.y + "\t" + v2.z + "\t" + vx0.x + "\t" + vx0.y + "\t" + vx0.z + "\t" + vy0.x + "\t" + vy0.y + "\t" + vy0.z + "\t" + vz0.x + "\t" + vz0.y + "\t" + vz0.z + "\t" + list.size() + "\t" + list.get(0).x + "\t" + list.get(0).y + "\t" + list.get(0).z + "\t" + inEllipsoid(v0Shifted,vx0) + "\t" + inEllipsoid(v0Shifted,vy0) + "\t" + inEllipsoid(v0Shifted,vz0) + "\t" + inEllipsoid(v0Shifted, vtarget));
//				else if (list.size() == 2)
//					System.out.println(a + "\t" + b + "\t" + c + "\t" + xShift + "\t" + yShift + "\t" + zShift + "\t" + x0 + "\t" + y0 + "\t" + z0 + "\t" + x2 + "\t" + y2 + "\t" + z2 + "\t" + elevation + "\t" + azimuth + "\t" + root1 + "\t" + root2 + "\t" + v1.x + "\t" + v1.y + "\t" + v1.z + "\t" + v2.x + "\t" + v2.y + "\t" + v2.z + "\t" + vx0.x + "\t" + vx0.y + "\t" + vx0.z + "\t" + vy0.x + "\t" + vy0.y + "\t" + vy0.z + "\t" + vz0.x + "\t" + vz0.y + "\t" + vz0.z + "\t" + list.size() + "\t" + list.get(0).x + "\t" + list.get(0).y + "\t" + list.get(0).z + "\t" + list.get(1).x + "\t" + list.get(1).y + "\t" + list.get(1).z + "\t" + inEllipsoid(v0Shifted,vx0) + "\t" + inEllipsoid(v0Shifted,vy0) + "\t" + inEllipsoid(v0Shifted,vz0) + "\t" + inEllipsoid(v0Shifted, vtarget));
//				else if (list.size() == 3)
//					System.out.println(a + "\t" + b + "\t" + c + "\t" + xShift + "\t" + yShift + "\t" + zShift + "\t" + x0 + "\t" + y0 + "\t" + z0 + "\t" + x2 + "\t" + y2 + "\t" + z2 + "\t" + elevation + "\t" + azimuth + "\t" + root1 + "\t" + root2 + "\t" + v1.x + "\t" + v1.y + "\t" + v1.z + "\t" + v2.x + "\t" + v2.y + "\t" + v2.z + "\t" + vx0.x + "\t" + vx0.y + "\t" + vx0.z + "\t" + vy0.x + "\t" + vy0.y + "\t" + vy0.z + "\t" + vz0.x + "\t" + vz0.y + "\t" + vz0.z + "\t" + list.size() + "\t" + list.get(0).x + "\t" + list.get(0).y + "\t" + list.get(0).z + "\t" + list.get(1).x + "\t" + list.get(1).y + "\t" + list.get(1).z + "\t" + list.get(2).x + "\t" + list.get(2).y + "\t" + list.get(2).z + "\t" + inEllipsoid(v0Shifted,vx0) + "\t" + inEllipsoid(v0Shifted,vy0) + "\t" + inEllipsoid(v0Shifted,vz0) + "\t" + inEllipsoid(v0Shifted, vtarget) + "\t" + vy0.equals(vx0));

				
				SLReporter.printInLog(Log.WARNING, "SLCrownFraction.intercept ()", "The number of interception if neither 0, 2 or an expected particular case. "
								+ "It could be a tangent point with the inner section. A check is needed! The list size is "
								+ list.size());
				SLReporter.printInStandardOutput("SLCrownFraction.intercept () - The number of interception if neither 0, 2 or an expected particular case. "
								+ "It could be a tangent point with the inner section. A check is needed! The list size is "
								+ list.size());
			}
			return null;
...
//			System.out.println(a + "\t" + b + "\t" + c + "\t" + xShift + "\t" + yShift + "\t" + zShift + "\t" + x0 + "\t" + y0 + "\t" + z0 + "\t" + x2 + "\t" + y2 + "\t" + z2 + "\t" + elevation + "\t" + azimuth + "\t" + root1 + "\t" + root2 + "\t" + v1.x + "\t" + v1.y + "\t" + v1.z + "\t" + v2.x + "\t" + v2.y + "\t" + v2.z + "\t" + vx0.x + "\t" + vx0.y + "\t" + vx0.z + "\t" + vy0.x + "\t" + vy0.y + "\t" + vy0.z + "\t" + vz0.x + "\t" + vz0.y + "\t" + vz0.z + "\t" + list.size() + "\t" + w1.x + "\t" + w1.y + "\t" + w1.z + "\t" + w2.x + "\t" + w2.y + "\t" + w2.z + "\t" + inEllipsoid(v0Shifted,vx0) + "\t" + inEllipsoid(v0Shifted,vy0) + "\t" + inEllipsoid(v0Shifted,vz0) + "\t" + inEllipsoid(v0Shifted, vtarget));
	...
	/**
	 * check if a vertex is included in the ellispoid of the crownPart (inside
	...
	public double getVolume() {
		...
		double volume = Math.abs(4d / 3d * Math.PI * a * b * c); // full ellipsoid
																// volume
		double fractionVolume = volume / 8d; // -> divided by 8
	...
	public double getLeafArea() {
		if (isSetLeafAreaDensity) {
			...
			SLReporter.printInLog(Log.WARNING, 
					"SLCrownFraction.getLeafArea(): this has been called before setting crown part LAD. It has returned 0.", null);
			return 0;
		}
	}
	...
	public boolean isIndsideCrownPart(double x, double y, double z) {
		...
		// gl-16.05.2019: I corrected the two following lines so as the bounding box depends on b and c also
		// this function was not used anymore and should be verified if it is used in the future.
		Vertex3d min = new Vertex3d (Math.min (x0, x0+a), Math.min (y0, y0+b), Math.min (z0, z0+c)); // box
		Vertex3d max = new Vertex3d (Math.max (x0, x0+a), Math.max (y0, y0+b), Math.max (z0, z0+c));
		boolean inBBox = (min.x <= v.x && v.x <= max.x && min.y <= v.y && v.y <= max.y && min.z <= v.z && v.z <= max.z);
