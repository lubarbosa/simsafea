...
/**
 * SLParaboloidalCrownFraction - For a crown made of 4 fractions of paraboloid.
 * 
 * @author B. Courbaud, N. Donès, M. Jonard, G. Ligot, F. de Coligny - October
 *         2008 / June 2012
 * This crown type was added by F. Andre - May 2019
 */
public class SLParaboloidalCrownFraction extends SLCrownPart {
	...
	/**
	 * Constructor for a 4th of a paraboloid. The signs of a and b determine
	 * which 4th is considered.
	 */
	public SLParaboloidalCrownFraction(double x0, double y0, double z0, double a, double b, double h) {
		this.x0 = x0;
		this.y0 = y0;
		this.z0 = z0;
		this.a = a;
	 ...
	 */
	public SLParaboloidalCrownFraction getCopy(double xShift, double yShift, double zShift) {

		SLParaboloidalCrownFraction copy = new SLParaboloidalCrownFraction(x0 + xShift, y0 + yShift, z0 + zShift, a, b, h);
	...
	// every years)
	public SLParaboloidalCrownFraction clone() throws CloneNotSupportedException {
		return (SLParaboloidalCrownFraction) super.clone();
	}
...
//		Vertex3d v0Shifted = new Vertex3d(x2,y2,z2);
		...
		if (inBBox(min, max, v0Shifted, v1, epsilon) && v1.z >= 0)
			list.add(v1);
		if (inBBox(min, max, v0Shifted, v2, epsilon) && v2.z >= 0)
			list.add(v2);
		if (inBBox(min, max, v0Shifted, vx0, epsilon) && inParaboloid(v0Shifted, vx0) && vx0.z > 0)
			list.add(vx0);
//		if (inBBox(min, max, v0Shifted, vy0, epsilon) && inParaboloid(v0Shifted, vy0) && vy0.z > 0)
		if (inBBox(min, max, v0Shifted, vy0, epsilon) && inParaboloid(v0Shifted, vy0) && vy0.z > 0 && !vy0.equals(vx0,epsilon)) //last condition is used to avoid duplicate point if beam intersects paraboloid axis, epsilon to consider rounding errors
			list.add(vy0);
//		if (inBBox(min, max, v0Shifted, vz0, epsilon) && inParaboloid(v0Shifted, vz0) && vz0.z > 0)
		if (inBBox(min, max, v0Shifted, vz0, epsilon) && inParaboloid(v0Shifted, vz0) && vz0.z > 0 && !vz0.equals(vx0,epsilon) && !vz0.equals(vy0,epsilon)) //two last conditions are used to avoid duplicate point if beam intersects paraboloid axis, epsilon to consider rounding errors
			list.add(vz0);
		if (inBBox(min, max, v0Shifted, vtarget, epsilon) && inParaboloid(v0Shifted, vtarget) && vtarget.z >= vz0.z)
			list.add(vtarget);
		if (list.size() == 0)
			return null; // intersected the paraboloid outside the quarter

		if (list.size() != 2) {

			// Particular cases
			boolean particularCase1 = (zTop2 == 0); // the top of the paraboloid is at the
													// same z as the
													// target point
...
//					System.out.println(a + "\t" + b + "\t" + h + "\t" + xShift + "\t" + yShift + "\t" + zShift + "\t" + x0 + "\t" + y0 + "\t" + z0 + "\t" + x2 + "\t" + y2 + "\t" + z2 + "\t" + zBottom2 + "\t" + zTop2 + "\t" + elevation + "\t" + azimuth + "\t" + root1 + "\t" + root2 + "\t" + v1.x + "\t" + v1.y + "\t" + v1.z + "\t" + v2.x + "\t" + v2.y + "\t" + v2.z + "\t" + vx0.x + "\t" + vx0.y + "\t" + vx0.z + "\t" + vy0.x + "\t" + vy0.y + "\t" + vy0.z + "\t" + vz0.x + "\t" + vz0.y + "\t" + vz0.z + "\t" + list.size() + "\t" + list.get(0).x + "\t" + list.get(0).y + "\t" + list.get(0).z + "\t" + list.get(1).x + "\t" + list.get(1).y + "\t" + list.get(1).z + "\t" + list.get(2).x + "\t" + list.get(2).y + "\t" + list.get(2).z + "\t" + inParaboloid(v0Shifted,vx0) + "\t" + inParaboloid(v0Shifted,vy0) + "\t" + inParaboloid(v0Shifted,vz0) + "\t" + inParaboloid(v0Shifted, vtarget));
				...
				SLReporter.printInLog(Log.WARNING, "SLParaboloidalCrownFraction.intercept ()", "The number of interception if neither 0, 2 or an expected particular case. "
								+ "It could be a tangent point with the inner section. A check is needed! The list size is "
								+ list.size());
				SLReporter.printInStandardOutput("SLParaboloidalCrownFraction.intercept () - The number of interception if neither 0, 2 or an expected particular case. "
								+ "It could be a tangent point with the inner section. A check is needed! The list size is "
								+ list.size());
			}
			return null;
...
//			System.out.println(a + "\t" + b + "\t" + xShift + "\t" + yShift + "\t" + zShift + "\t" + x0 + "\t" + y0 + "\t" + z0 + "\t" + x2 + "\t" + y2 + "\t" + z2 + "\t" + zBottom2 + "\t" + zTop2 + "\t" + elevation + "\t" + azimuth + "\t" + root1 + "\t" + root2 + "\t" + v1.x + "\t" + v1.y + "\t" + v1.z + "\t" + v2.x + "\t" + v2.y + "\t" + v2.z + "\t" + vx0.x + "\t" + vx0.y + "\t" + vx0.z + "\t" + vy0.x + "\t" + vy0.y + "\t" + vy0.z + "\t" + vz0.x + "\t" + vz0.y + "\t" + vz0.z + "\t" + list.size() + "\t" + w1.x + "\t" + w1.y + "\t" + w1.z + "\t" + w2.x + "\t" + w2.y + "\t" + w2.z + "\t" + inParaboloid(v0Shifted,vx0) + "\t" + inParaboloid(v0Shifted,vy0) + "\t" + inParaboloid(v0Shifted,vz0) + "\t" + inParaboloid(v0Shifted, vtarget));
	...
	/**
	 * check if a vertex is included in the paraboloid of the crownPart (inside
	...
	public double getVolume() {
		...
		double volume = Math.abs(1d / 2d * Math.PI * a * b * h); // full paraboloid volume
	...
	public double getLeafArea() {
		if (isSetLeafAreaDensity) {
			...
			SLReporter.printInLog(Log.WARNING, 
					"SLParaboloidalCrownFraction.getLeafArea(): this has been called before setting crown part LAD. It has returned 0.", null);
			return 0;
		}
	}
